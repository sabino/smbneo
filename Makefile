...
.PHONY: \
	all ci test web-test verify codegen cart compat-cart hardware-cart neosd-cart run web \
...
cart compat-cart hardware-cart neosd-cart run web mame-list mame-cart mame-run mame-capture \
replay-cart replay-run replay-rendered-evidence clean:
