...
def parse_fm2(data: bytes, source: str = "<memory>") -> Fm2Movie:
    ...
    case-sensitive because letter case changes the encoded MD5 bytes.
    """
    ...
    try:
        ...
        raise RecordingError(
            f"{source}: FM2 must be UTF-8 text; binary or invalid data begins "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "import-fm2":
